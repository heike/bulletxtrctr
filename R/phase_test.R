...
#' \eqn{T = \bar{X}_{SS} - \tilde{X}_{DS}}, where groups SS and DS are set by phase selection (after inspecting the set of n values).
...
#' where F is the (approximately normal) distribution of \eqn{\bar{X}_{SS}} under \eqn{H_0} with expected value of \eqn{\mu} (\eqn{=E[\bar{X}_{SS}] = E[\bar{X}_{DS}]}) and standard deviation \eqn{\sigma} (standard deviation of a group of size $n$ under $H_0$).
#' @param t value of the observed test statistic, t has to be non-negative.
